...
#endif
{
	...
	GetModuleFileNameA(NULL, curr_dir, 255);
	strcpy(curr_dir, "C:\\Program Files(x86)\\Diablo II");
	...
	handle = CreateFile(patch_d2, FILE_GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
	...
	sprintf(d2gsconf.versionMotd, "歡迎進入91D2暗黑戰網暗黑1.13c遊戲服務器，軟件編譯時間：%s", BUILDDATE);
...
void dump_callstack(CONTEXT *context)
{
	...
	fprintf(dump, "Fatal Address: %#lu", context->Eip);
	fflush(dump);
	fprintf(dump, "rax: %#lu  rbx: %#lu  rcx: %#lu  rdx: %#lu  flg: %#lu", context->Eax, context->Ebx, context->Ecx, context->Edx, context->EFlags);
	fflush(dump);
	fprintf(dump, "rbp: %#lu  rdi: %#lu  rip: %#lu  rsi: %#lu  rsp: %#lu", context->Ebp, context->Edi, context->Eip, context->Esi, context->Esp);
	fflush(dump);
	...
	for (; ; )
	{
		...
		fprintf(dump, "  Call stack: %#lu", sf.AddrPC.Offset);
		fflush(dump);

		if (SymFromAddr(hProcess, sf.AddrPC.Offset, 0, pSymbol))
		{
			fprintf(dump, "    Symbol: %s (base: %#llu)", pSymbol->Name, pSymbol->ModBase);
			fflush(dump);
		}
		...
		{
			fprintf(dump, "    Source: Line %lu in %s", lineInfo.LineNumber, lineInfo.FileName);
			fflush(dump);
		}
	}
