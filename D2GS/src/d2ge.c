...
DWORD WINAPI					GEListenLoop(LPVOID lpParam);
...
int D2GEStartup(void)
{
	...
	for (size_t i = strlen(curr_dir) - 1; i >= 0; i--)
	{
		if (curr_dir[i] == '\\') {
			curr_dir[i] = 0;
			break;
	...
	for (DWORD i = 0; i < d2gsconf.gethread; i++)
	{
...
DWORD WINAPI GEListenLoop(LPVOID lpParam)
{
	...
	while (1)
	{
		...
		if (AcceptSocket == INVALID_SOCKET)
			continue;
		hThread = CreateThread(NULL, 0, GECommThread, (LPVOID)AcceptSocket, 0, &dwThreadId);
		if (hThread)
			CloseHandle(hThread);
	}
}
