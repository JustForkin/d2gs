...
DWORD WINAPI TrunkLoop(LPVOID lpParam);
...
BOOL StartupTrunk()
{
	...
	if (bind(strunk, (struct sockaddr*)&srv_addr, sizeof(struct sockaddr_in)) == SOCKET_ERROR)
		return FALSE;
	listen(strunk, 3);
	hThread = CreateThread(NULL, 0, TrunkLoop, NULL, 0, &dwThreadId);
	if (hThread)
		CloseHandle(hThread);
	return TRUE;
}
...
DWORD WINAPI TrunkLoop(LPVOID lpParam)
{
	...
	while (1)
	{
		...
		if (AcceptSocket == INVALID_SOCKET)
			continue;
		hThread = CreateThread(NULL, 0, TrunkClientLoop, (LPVOID)AcceptSocket, 0, &dwThreadId);
		if (hThread)
			CloseHandle(hThread);
	}
