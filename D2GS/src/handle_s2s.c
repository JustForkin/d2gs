...
void D2CSSetConfFile(LPVOID *lpdata)
{
	...
	if(strcmp(buf, "1") == 0){
		...
		if(strcmp(buf2, "0") == 0){
			sprintf(d2gsconf.eventmotd, "%sDCÓ|°lÄ£Ê½£ºÈ«‡ø¶ÈÓ|°l¡£DC KeyÎïÆ·žé%s¡£", "%yellow%", buf);
		}else{
			dcrate = atoi(buf2) / 10;
			sprintf(d2gsconf.eventmotd, "%sDCÓ|°lÄ£Ê½£º†Î·¿égÓ|°l¡£DC KeyÎïÆ·žé%s£¬Ó|°l¸ÅÂÊ%d%%¡£", "%yellow%", buf, dcrate);
		}
	}else{
		sprintf(d2gsconf.eventmotd, "%s®”Ç°·þ„ÕÆ÷²»Ö§³ÖDCÓ|°l", "%yellow%", buf);
	}*/
	GetPrivateProfileString("World Event", "Item", "key", buf, 255, confpath);
	...
	WORD wCharClass, DWORD dwCharLevel)
{
	...
	if (lpChar)
	{
		pUpdateInfo->addr = bn_htonl(inet_addr(lpChar->IpAddr));
	}
	strcpy((packet.data) + sizeof(t_d2gs_d2cs_updategameinfo), lpCharName);
	packet.datalen = sizeof(t_d2gs_d2cs_updategameinfo) + strlen(lpCharName) + 1;
	packet.peer = PACKET_PEER_SEND_TO_D2CS;
